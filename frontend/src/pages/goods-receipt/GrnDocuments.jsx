...
export default function GrnDocuments() {
    ...
    function handlePrint(grn) {
        ...
        if (!printWindow) {
            push('Allow pop-ups for this site to print the GRN document.', 'error')
            return
        }
        ...
        <div className="space-y-4">
            ...
            <div className="mb-6">
                <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
                    ...
                    <div>
                        ...
                        <p className="font-medium text-success-700">{grn.status === GRN_STATUS.POSTED ? 'Accepted, Generated & Posted' : 'Accepted & Generated'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="border-t border-ink-200 pt-4 grid grid-cols-3 gap-6 text-sm">
                ...
                <div>
                    ...
                    <p className="border-t border-ink-400 pt-2 text-center text-ink-600">
                        {formatDate(grn.generatedAt || grn.receivedDate)}
                    </p>
                </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-ink-200 text-xs text-ink-500 text-center">
                ...
                <p>Document Reference: {grn.grnRef} | Generated by: {grn.generatedBy || 'Storekeeper'} | Generated: {formatDate(grn.generatedAt || grn.receivedDate)}</p>
            </div>
        </div>
    )
}
...
function renderPrintableGRN(grn) {
    const acceptedQuantity = (item) => item.qtyAccepted == null ? Number(item.qty) : Number(item.qtyAccepted)
    const itemTotal = (grn.items || []).reduce((sum, item) => sum + (acceptedQuantity(item) * Number(item.unitPrice)), 0)
    ...
    <body>
      ...
      <table>
        ...
        <tbody>
          ${(grn.items || []).map((item) => `
            <tr>
              <td>${item.item}</td>
              <td class="text-center">${acceptedQuantity(item)}</td>
              <td class="text-right">${formatCurrency(item.unitPrice)}</td>
              <td class="text-right"><strong>${formatCurrency(acceptedQuantity(item) * Number(item.unitPrice))}</strong></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <div class="signatures">
        ...
        <div>
          ...
          <div class="sig-line">${formatDate(grn.generatedAt || grn.receivedDate)}</div>
        </div>
      </div>
